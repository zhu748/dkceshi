...
func TestMessagesPrepareBasic(t *testing.T) {
        ...
        // Output integrity guard 默认开启，prompt 以 system 块开头。
        if !strings.HasPrefix(got, "<|begin▁of▁sentence|><|System|>") {
                t.Fatalf("expected system block at the start (guard enabled by default), got %q", got)
        }
...
func TestMessagesPrepareRoles(t *testing.T) {
        ...
        got := MessagesPrepare(messages)
        // Output integrity guard 默认开启。
        if !contains(got, "Output integrity guard") {
                t.Fatalf("output integrity guard should be enabled by default, got %q", got)
        }
