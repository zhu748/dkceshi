...
func NormalizeOpenAIChatRequest(store ConfigReader, req map[string]any, traceID string) (StandardRequest, error) {
        ...
        return StandardRequest{
                ...
                EditReuseEnabled:    config.IsEditReuseModel(resolvedModel),
                MessagesFingerprint: computeMessagesFingerprint(messagesRaw),
                LastUserMessage:     extractLastUserMessage(messagesRaw),
        }, nil
}
...
// computeMessagesFingerprint 计算 messages 前 N-1 条的 SHA-256 指纹。
// 只有前 N-1 条一致时才认为是同一对话的延续，最后一条是新消息。
func computeMessagesFingerprint(messages []any) string {
        if len(messages) <= 1 {
                return ""
        }
...
// extractLastUserMessage 提取 messages 中最后一条 role=user 的文本内容。
// edit_message 只发送这段文字，而非完整对话历史。
