...
const (
        ...
        DeepSeekEditMessageURL       = "https://chat.deepseek.com/api/v0/chat/edit_message"
        DeepSeekCompletionTargetPath = "/api/v0/chat/completion"
        DeepSeekUploadTargetPath     = "/api/v0/file/upload_file"
)

// defaultStaticBaseHeaders 仅包含 App 实际发送的静态头。
// 注意：HTTP/2 下 Host 由 :authority pseudo-header 自动承载，无需显式设置。
...
func buildBaseHeaders(client clientConstants, overrides map[string]string) map[string]string {
        ...
        if client.Locale != "" {
                ...
                // 注意：真实 Android App 不发送 Accept-Language 头，仅发送 x-client-locale，此处保持对齐。
        }
