...
func (m *vercelReleaseAutoDeleteDSStub) CallCompletion(_ context.Context, _ *auth.RequestAuth, _ any, _ string, _ int) (*http.Response, error) {
	return m.resp, nil
}
...
func TestHandleVercelStreamPrepareUploadsToolsSeparately(t *testing.T) {
	...
	if strings.Contains(string(ds.uploadCalls[0].Data), "Synopsis: search docs") {
...
func TestHandleVercelStreamSwitchReuploadsCurrentInputFile(t *testing.T) {
	...
	stdReq := promptcompat.StandardRequest{
		...
		FinalPrompt:             "Resume from the latest snapshot in chat_context.txt. The callable function definitions and parameter contracts are attached in tool_schema.txt; only depend on the functions and parameters enumerated there and follow the function-call contract specified in this prompt.",
		PromptTokenText:         "# Session Snapshot\n\n=== 1. USER ===\nhello\n\n# Callable Surface\nCatalogue of invokable functions and their parameter contracts for this turn.\n\nThese tools are available for this turn:\n\nCallable: search\nSynopsis: search docs\nContract: {\"type\":\"object\"}\n",
		HistoryText:             "# Session Snapshot\n\n=== 1. USER ===\nhello\n",
	...
	promptText, _ := payload["prompt"].(string)
	if !strings.Contains(promptText, "tool_schema.txt") {
