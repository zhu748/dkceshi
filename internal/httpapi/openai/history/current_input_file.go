...
func currentInputFilePrompt(hasToolsFile bool) string {
	prompt := "Resume from the latest snapshot in chat_context.txt. Take that file as the authoritative working state and answer the most recent user request directly."
	if hasToolsFile {
		prompt += " The callable function definitions and parameter contracts are attached in tool_schema.txt; only depend on the functions and parameters enumerated there and follow the function-call contract specified in this prompt."
	}
	return prompt
}
