...
func TestBuildOpenAICurrentInputContextTranscriptUsesNumberedHistorySections(t *testing.T) {
	...
	if !strings.Contains(transcript, "# Session Snapshot") {
		t.Fatalf("expected history transcript header, got %q", transcript)
	}
	if !strings.Contains(transcript, "Aggregated dialogue state and prior function-call results.") {
...
func TestApplyCurrentInputFileUploadsFirstTurnWithNumberedHistoryTranscript(t *testing.T) {
	...
	if upload.Filename != "chat_context.txt" {
	...
	if !strings.Contains(out.FinalPrompt, "Resume from the latest snapshot in chat_context.txt.") {
	...
	if !strings.Contains(out.PromptTokenText, "# Session Snapshot") || !strings.Contains(out.PromptTokenText, "=== 1. USER ===") {
...
func TestApplyCurrentInputFilePreservesFullContextPromptForTokenCounting(t *testing.T) {
	...
	if !strings.Contains(out.PromptTokenText, "# Session Snapshot") || !strings.Contains(out.PromptTokenText, "=== 1. SYSTEM ===") {
		t.Fatalf("expected prompt token text to include numbered history transcript, got %q", out.PromptTokenText)
	}
	if !strings.Contains(out.PromptTokenText, "Resume from the latest snapshot in chat_context.txt.") {
...
func TestApplyCurrentInputFileUploadsFullContextFile(t *testing.T) {
	...
	upload := ds.uploadCalls[0]
	if upload.Filename != "chat_context.txt" {
		t.Fatalf("expected chat_context.txt upload, got %q", upload.Filename)
	}
	if upload.ModelType != "vision" {
		t.Fatalf("expected vision model type for vision request, got %q", upload.ModelType)
	}
	uploadedText := string(upload.Data)
	for _, want := range []string{"# Session Snapshot", "=== 1. SYSTEM ===", "=== 2. USER ===", "=== 3. ASSISTANT ===", "=== 4. TOOL ===", "=== 5. USER ===", "system instructions", "first user turn", "hidden reasoning", "tool result", "latest user turn", promptcompat.ThinkingInjectionMarker} {
		if !strings.Contains(uploadedText, want) {
...
func TestApplyCurrentInputFileUploadsToolsContextSeparately(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "chat_context.txt" {
		t.Fatalf("expected first upload to be chat_context.txt, got %q", ds.uploadCalls[0].Filename)
	}
	if ds.uploadCalls[1].Filename != "tool_schema.txt" {
		t.Fatalf("expected second upload to be tool_schema.txt, got %q", ds.uploadCalls[1].Filename)
	}
	...
	for _, want := range []string{"# Callable Surface", "Callable: search", "Synopsis: search docs", `Contract: {"type":"object"}`} {
		if !strings.Contains(toolsText, want) {
			t.Fatalf("expected tools transcript to contain %q, got %q", want, toolsText)
		}
	}
	if strings.Contains(toolsText, "FUNCTION INVOCATION CONTRACT") {
		t.Fatalf("tools transcript should not duplicate function-call format instructions, got %q", toolsText)
	}
	if !strings.Contains(out.FinalPrompt, "Resume from the latest snapshot in chat_context.txt.") || !strings.Contains(out.FinalPrompt, "tool_schema.txt") {
		t.Fatalf("expected live prompt to reference both context files, got %q", out.FinalPrompt)
	}
	if !strings.Contains(out.FinalPrompt, "FUNCTION INVOCATION CONTRACT") || !strings.Contains(out.FinalPrompt, "Reminder: The ONLY sanctioned way to invoke a function") {
		t.Fatalf("expected live prompt to retain function-call format instructions, got %q", out.FinalPrompt)
	}
	if strings.Contains(out.FinalPrompt, "These tools are available for this turn") || strings.Contains(out.FinalPrompt, "Synopsis: search docs") || strings.Contains(out.FinalPrompt, "Contract:") {
	...
	if !strings.Contains(out.PromptTokenText, "# Session Snapshot") || !strings.Contains(out.PromptTokenText, "# Callable Surface") || !strings.Contains(out.PromptTokenText, "Synopsis: search docs") {
...
func TestApplyCurrentInputFileCarriesHistoryText(t *testing.T) {
	...
	if !strings.Contains(out.HistoryText, "# Session Snapshot") || !strings.Contains(out.HistoryText, "=== 1. SYSTEM ===") {
...
func TestChatCompletionsCurrentInputFileUploadsContextAndKeepsNeutralPrompt(t *testing.T) {
	...
	if !strings.Contains(historyText, "# Session Snapshot") || !strings.Contains(historyText, "=== 1. SYSTEM ===") {
...
func TestResponsesCurrentInputFileUploadsToolsSeparately(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "chat_context.txt" || ds.uploadCalls[1].Filename != "tool_schema.txt" {
		t.Fatalf("unexpected upload filenames: %#v", ds.uploadCalls)
	}
	historyText := string(ds.uploadCalls[0].Data)
	if strings.Contains(historyText, "Synopsis: search docs") {
		t.Fatalf("history transcript should not embed tool descriptions, got %q", historyText)
	}
	toolsText := string(ds.uploadCalls[1].Data)
