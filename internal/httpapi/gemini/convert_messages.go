...
const (
        maxGeminiRawPromptChars  = 1024
        geminiOmittedBinaryTag  = "[binary_payload_omitted]"
        geminiTruncationSuffix  = "...(content trimmed)"
)
...
func formatGeminiUnknownPartForPrompt(part map[string]any) string {
        ...
        if len(raw) > maxGeminiRawPromptChars {
                return raw[:maxGeminiRawPromptChars] + geminiTruncationSuffix
        }
        return raw
}
...
func sanitizeGeminiStringForPrompt(key, value string) string {
        ...
        if looksLikeGeminiBinaryField(key) || looksLikeGeminiBase64(trimmed) {
                return geminiOmittedBinaryTag
        }
        if len(trimmed) > maxGeminiRawPromptChars {
                return trimmed[:maxGeminiRawPromptChars] + geminiTruncationSuffix
        }
        return trimmed
}
