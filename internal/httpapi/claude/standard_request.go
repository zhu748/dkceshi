...
func normalizeClaudeRequest(store ConfigReader, req map[string]any) (claudeNormalizedRequest, error) {
        ...
        // 与 OpenAI 路径对齐：hasTools 以最终提取出的 toolNames 是否非空为准，
        // 用于决定是否注入 Output integrity guard。
        hasTools := len(toolNames) > 0
        finalPrompt := prompt.MessagesPrepareWithThinkingAndToolHint(toMessageMaps(dsPayload["messages"]), thinkingEnabled, hasTools)
