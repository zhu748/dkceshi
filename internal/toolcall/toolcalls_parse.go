...
func parseToolCallsDetailedXMLOnly(text string) ToolCallParseResult {
        ...
        if !ok {
                // DSML 规范化失败，尝试 Claude Code 兜底（Tool: X\n<tool_input>{...}</tool_input> 等）
                // 这种情况说明模型没有按 DSML 规则输出，而是用了它训练偏好的格式。
                // 兜底转换后再解析，避免工具调用被当成普通文本卡住。
                if fallback, matched := tryParseClaudeCodeFallback(trimmed); matched {
                        return fallback
                }
                return result
        }
        ...
        if len(parsed) == 0 {
                // 标准 DSML 解析失败，尝试 Claude Code 兜底
                if fallback, matched := tryParseClaudeCodeFallback(trimmed); matched {
                        return fallback
                }
                return result
