...
// ThinkingInjectionEnabled 返回全局思考注入开关。
// 默认关闭（nil → false）：思考注入应按需启用，避免对所有请求一律注入
// "Reasoning Effort: Absolute maximum..." 提示词导致行为变化。
// 启用方式：
//   - 全局打开：config 中显式设置 thinking_injection.enabled=true
//   - 按请求打开：使用带 -thinkinginject 后缀的模型 ID
