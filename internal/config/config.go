...
type Config struct {
        ...
        OutputIntegrityGuard OutputIntegrityGuardConfig  `json:"output_integrity_guard,omitempty"`
...
// OutputIntegrityGuardConfig 控制 prompt 前置注入的 "Output integrity guard:"
// 系统消息。早期实现每次请求都注入，是强 system 头指纹；现在默认关闭，
// 可通过 config 显式启用。
type OutputIntegrityGuardConfig struct {
        Enabled *bool `json:"enabled,omitempty"`
}
