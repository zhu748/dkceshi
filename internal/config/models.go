...
func buildOllamaCapabilitiesModels() []OllamaCapabilitiesModelInfo {
	...
	for _, base := range deepSeekBaseModels {
		for _, combo := range suffixCombinations {
			id := withSuffixes(base.ID, combo.noThinking, combo.forceHistory, combo.autoDelete, combo.thinkingInject)
...
func GetModelConfig(model string) (thinking bool, search bool, ok bool) {
	baseModel, noThinking, _, _, _ := parseModelSuffixes(model)
	if baseModel == "" {
...
func GetModelType(model string) (modelType string, ok bool) {
	baseModel, _, _, _, _ := parseModelSuffixes(model)
	switch baseModel {
...
func IsNoThinkingModel(model string) bool {
	_, noThinking, _, _, _ := parseModelSuffixes(model)
	return noThinking
}
...
func IsAutoDeleteModel(model string) bool {
	_, _, _, autoDelete, _ := parseModelSuffixes(model)
	return autoDelete
}
...
func IsForceHistoryModel(model string) bool {
	_, _, forceHistory, _, _ := parseModelSuffixes(model)
	return forceHistory
}
...
func IsThinkingInjectModel(model string) bool {
	_, _, _, _, thinkingInject := parseModelSuffixes(model)
...
func ResolveModel(store ModelAliasReader, requested string) (string, bool) {
	...
	baseModel, noThinking, forceHistory, autoDelete, thinkingInject := parseModelSuffixes(model)
	if baseModel != model {
		// 用户传入了带后缀的形式（如 gpt-4.1-nothinking / gpt-4.1-autodelete /
		// gpt-4.1-forcehistory / gpt-4.1-forcehistory-autodelete-thinkinginject 等任意组合）。
		if mapped, ok := aliases[baseModel]; ok && IsSupportedDeepSeekModel(mapped) {
			...
			mappedBase, mappedNoThinking, mappedForceHistory, mappedAutoDelete, mappedThinkingInject := parseModelSuffixes(mapped)
			return withSuffixes(
				mappedBase,
...
func appendModelVariants(models []ModelInfo) []ModelInfo {
	...
	for _, model := range models {
		base, _, _, _, _ := parseModelSuffixes(model.ID)
		for _, combo := range suffixCombinations {
			variant := model
			variant.ID = withSuffixes(base, combo.noThinking, combo.forceHistory, combo.autoDelete, combo.thinkingInject)
...
func parseModelSuffixes(model string) (base string, noThinking, forceHistory, autoDelete, thinkingInject bool) {
	...
	return model, noThinking, forceHistory, autoDelete, thinkingInject
}
...
func withSuffixes(base string, noThinking, forceHistory, autoDelete, thinkingInject bool) string {
	if base == "" {
		return ""
	}
	if noThinking {
...
func splitNoThinkingModel(model string) (string, bool) {
	base, noThinking, _, _, _ := parseModelSuffixes(model)
	return base, noThinking
}

func withNoThinkingVariant(model string, enabled bool) string {
	base, _, forceHistory, autoDelete, thinkingInject := parseModelSuffixes(model)
	return withSuffixes(base, enabled, forceHistory, autoDelete, thinkingInject)
}
...
func splitAutoDeleteModel(model string) (string, bool) {
	base, _, _, autoDelete, _ := parseModelSuffixes(model)
	return base, autoDelete
}

func withAutoDeleteVariant(model string, enabled bool) string {
	base, noThinking, forceHistory, _, thinkingInject := parseModelSuffixes(model)
	return withSuffixes(base, noThinking, forceHistory, enabled, thinkingInject)
}
...
func splitForceHistoryModel(model string) (string, bool) {
	base, _, forceHistory, _, _ := parseModelSuffixes(model)
	return base, forceHistory
}

func withForceHistoryVariant(model string, enabled bool) string {
	base, noThinking, _, autoDelete, thinkingInject := parseModelSuffixes(model)
	return withSuffixes(base, noThinking, enabled, autoDelete, thinkingInject)
}
