...
func (f *fakeDeepSeekCaller) CallCompletion(_ context.Context, a *auth.RequestAuth, payload any, _ string, _ int) (*http.Response, error) {
...
func TestExecuteNonStreamWithRetryReuploadsCurrentInputFileAfterAccountSwitch(t *testing.T) {
        ...
        refIDs, _ := asMap(ds.payloads[2])["ref_file_ids"].([]any)
        if len(refIDs) != 1 || refIDs[0] != "file-runtime-acc2@test.com" {
                t.Fatalf("expected switched account ref_file_ids to use reuploaded file, got %#v", asMap(ds.payloads[2])["ref_file_ids"])
        }
}
...
func TestExecuteNonStreamWithRetryUsesParentMessageForEmptyRetry(t *testing.T) {
        ...
        if got := asMap(ds.payloads[1])["parent_message_id"]; got != 77 {
