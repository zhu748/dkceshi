...
func TestExecuteStreamWithRetryUsesSharedRetryPayloadAndUsagePrompt(t *testing.T) {
        ...
        if got := asMap(ds.payloads[0])["parent_message_id"]; got != 77 {
                t.Fatalf("retry parent_message_id mismatch: %#v", got)
        }
        if prompt, _ := asMap(ds.payloads[0])["prompt"].(string); !strings.Contains(prompt, shared.EmptyOutputRetrySuffix) {
...
func TestExecuteStreamWithRetrySwitchesManagedAccountBeforeFinal429(t *testing.T) {
        ...
        if got := asMap(ds.payloads[1])["chat_session_id"]; got != "session-acc2@test.com" {
                t.Fatalf("switched payload session mismatch: %#v", got)
        }
        if prompt, _ := asMap(ds.payloads[1])["prompt"].(string); strings.Contains(prompt, shared.EmptyOutputRetrySuffix) {
