...
func ExecuteStreamWithRetry(ctx context.Context, ds DeepSeekCaller, a *auth.RequestAuth, initialResp *http.Response, payload any, pow string, opts StreamRetryOptions, hooks StreamRetryHooks) {
...
func startPayloadCompletionOnAlternateAccount(ctx context.Context, ds DeepSeekCaller, a *auth.RequestAuth, payload any, opts StreamRetryOptions, maxAttempts int) (StartResult, *assistantturn.OutputError) {
        ...
        // 如果 payload 是 *OrderedJSONMap，用 Set/Delete 保留顺序；否则 fallback 到 map 操作
        if m, ok := nextPayload.(*promptcompat.OrderedJSONMap); ok {
                m.Set("chat_session_id", sessionID)
                m.Delete("parent_message_id")
        } else if mm, ok := nextPayload.(map[string]any); ok {
                mm["chat_session_id"] = sessionID
                delete(mm, "parent_message_id")
        }
